...
class SkillGateway:
    ...
    # Embedder is lazy-loaded — first search() call pays the cost (~2s),
    # subsequent calls reuse the cached model. None means we tried and
    # failed, so don't keep retrying.
    _UNSET = object()
    ...
    def __init__(self, skills_dir: Optional[Path] = None):
        ...
        # (skill_name → 384-dim numpy vector). Built on first search().
        self._skill_embeddings: Dict[str, Any] = {}
        # Sentinel: not-yet-loaded vs known-unavailable.
        self._embedder = self._UNSET
        logger.info("SkillGateway initialized with %d skill packages", len(self._packages))

    def reload_packages(self):
        """Reload all skill packages from disk and invalidate the embedding cache."""
        from gaia_mcp.skill_package import load_all_packages
        self._packages = load_all_packages(self._skills_dir)
    ...
    def _get_embedder(self):
        ...
        if self._embedder is self._UNSET:
            try:
                ...
                # Container path first, host path as fallback.
                import os as _os
                for path in ("/models/all-MiniLM-L6-v2",
                             "/gaia/gaia-instance/gaia-models/all-MiniLM-L6-v2"):
                    if _os.path.isdir(path):
                        self._embedder = SentenceTransformer(path)
                        logger.info("SkillGateway: embedder loaded from %s", path)
                        break
                else:
                    logger.warning("SkillGateway: embedder model not found, falling back to keyword matching")
                    self._embedder = None
            except Exception as e:
                logger.warning("SkillGateway: embedder load failed (%s), falling back to keyword matching", e)
                self._embedder = None
        return self._embedder

    def _get_skill_utility(self, skill_name: str) -> float:
        """Per-skill utility score in [0, 1]. 0.5 = neutral / no history.

        Reads the same persistent store knowledge_router.record_outcome
        writes to. Keyed under domain 'skill' so it doesn't collide with
        knowledge-source utility scores.
        """
        try:
            import json as _json
            import os as _os
            path = "/shared/knowledge_router/utility_scores.json"
            if not _os.path.exists(path):
                return 0.5
            with open(path) as f:
                data = _json.load(f)
            return float(data.get(f"skill:{skill_name}", 0.5))
        except Exception:
            return 0.5

    def _build_skill_embeddings(self) -> None:
        """Compute one embedding per skill using 'name + description' as text."""
        ...
        try:
            ...
            self._skill_embeddings = {n: v for n, v in zip(names, vecs)}
            logger.info("SkillGateway: built embeddings for %d skills", len(names))
        except Exception as e:
            logger.warning("SkillGateway: embedding build failed: %s", e)
            self._skill_embeddings = {}
    ...
    # Cosine similarity below this is too weak to be a real match — at
    # this threshold all-MiniLM-L6-v2 still surfaces structurally
    # related skills but stops returning everything for unrelated queries.
    _MIN_SKILL_SIMILARITY = 0.30

    def _find_matching_skills(self, query: str, top_k: int = 5) -> list:
        """Skill discovery via sentence-transformer embedding similarity.

        Falls back to keyword overlap when the embedder is unavailable
        (e.g. the model directory isn't mounted). Returns
        list of (skill_name, score) tuples sorted by descending score.
        """
        if not self._packages:
            return []

        # Build the embedding cache lazily on first call.
        if not self._skill_embeddings:
            self._build_skill_embeddings()

        embedder = self._get_embedder()
        if embedder is not None and self._skill_embeddings:
            try:
                import numpy as np
                qvec = embedder.encode([query], normalize_embeddings=True)[0]
                scores = []
                for name, svec in self._skill_embeddings.items():
                    sim = float(np.dot(qvec, svec))
                    if sim < self._MIN_SKILL_SIMILARITY:
                        continue
                    # Per-skill utility score blend. Skills that have failed
                    # often get downweighted; skills with no history score
                    # 0.5 (neutral) and so are unaffected. Blend is 80/20:
                    # similarity dominates, utility breaks ties between
                    # equally-relevant skills.
                    utility = self._get_skill_utility(name)
                    blended = (sim * 0.8) + (utility * 0.2)
                    scores.append((name, blended))
                scores.sort(key=lambda x: x[1], reverse=True)
                return scores[:top_k]
            except Exception as e:
                logger.warning("SkillGateway: embedding match failed (%s), falling back to keywords", e)
    ...
    async def _handle_learn(self, params: Dict) -> Dict:
        ...
        skill_name = params.get("skill")  # if a specific skill ran

        if isinstance(success, str):
            success = success.lower() in ("true", "1", "yes")

        # Record utility outcome — both for the source (knowledge-router
        # convention) and for the specific skill if one was named.
        try:
            ...
            if skill_name:
                # Skill utility lives under 'skill' domain so it doesn't
                # mingle with source-level scoring.
                record_outcome("skill", skill_name, success)
            ...
            # MCP isolated from gaia-core — fall back to a direct write so
    ...
    def _direct_record_skill_outcome(self, skill_name: str, success: bool) -> None:
        """Fallback when knowledge_router isn't importable — write directly."""
        try:
            import json as _json
            import os as _os
            path = "/shared/knowledge_router/utility_scores.json"
            ...
            current = float(data.get(key, 0.5))
            data[key] = min(1.0, current + 0.1) if success else max(0.0, current - 0.15)
            _os.makedirs(_os.path.dirname(path), exist_ok=True)
            with open(path, "w") as f:
                _json.dump(data, f, indent=2)
        except Exception as e:
            logger.debug("SkillGateway: direct skill outcome write failed: %s", e)
