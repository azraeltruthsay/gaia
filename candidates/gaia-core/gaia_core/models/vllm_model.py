...
class VLLMChatModel:
    ...
    def create_completion(
        ...
        **kwargs,
        ...
        if text.strip().lower() in tool_noise:
            logger.debug("VLLMChatModel.create_completion filtered tool-like text: %r", text)
            text = ""
        logger.debug("VLLMChatModel.create_completion raw outputs: %s", self._summarize_outputs(outputs))
    ...
    def _create_chat_completion_simple(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.debug("VLLMChatModel._create_chat_completion_simple prompt: %s", prompt)
    ...
    def _native_stream_text(self, prompts: List[str], sampling_params: SamplingParams) -> Iterator[Dict[str, Any]]:
        ...
        for chunk in generator:
            ...
            logger.debug(f"VLLM NATIVE STREAM CHUNK: {chunk}")
            logger.debug(f"VLLM NATIVE STREAM TEXT: {text}")
            logger.debug(f"VLLM NATIVE STREAM PREV: {prev}")
            logger.debug(f"VLLM NATIVE STREAM DELTA: {delta}")
            cache[req_id] = text
            if not delta:
                continue
            any_delta = True
