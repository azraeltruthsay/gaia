...
def update_seed(seed_id: str, seed_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        logger.error(f"❌ Error updating thought seed file {seed_file}: {e}", exc_info=True)
        return False
...
def refine_seed(seed_id, refinement_prompt, config=None, llm=None):
    ...
    if llm is None:
        if hasattr(config, 'model_pool') and config.model_pool is not None:
            ...
            logger.error("❌ No LLM provided and model_pool not available in config for thought seed refinement.")
            return None
...
New Seed:"""
    ...
    try:
        ...
        logger.error(f"❌ Error refining thought seed {seed_id}: {e}", exc_info=True)
        return None
