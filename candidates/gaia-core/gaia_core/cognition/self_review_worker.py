...
def run_review_once(config: Config = None):
	...
	from gaia_common.protocols.cognition_packet import CognitionPacket, Persona, Origin, Routing, Model, Header, Content, Context, Constraints
	from gaia_core.config import Config, get_config
			...
			if mark and task_name:
				...
				for item in dev:
					if item.get("task") == task_name:
						...
						item["resolved"] = datetime.utcnow().isoformat()
						if "audit" not in item:
							item["audit"] = []
						item["audit"].append({"by": "gaia_self_review", "justification": justification, "ts": datetime.utcnow().isoformat()})
						matched = True
						break
				if not matched:
					# Append a new resolved task entry
					dev.append({"task": task_name, "status": "resolved", "resolved": datetime.utcnow().isoformat(), "audit": [{"by": "gaia_self_review", "justification": justification, "ts": datetime.utcnow().isoformat()}]})
...
def run_review_with_prompt(prompt: str, task_key: str = "thought_seed_system", session_id: str = "rescue-shell", persona_id: str = "RescueOperator"):
	...
	routing = Routing(target_engine="Lite", allow_parallel=False, priority=5)
