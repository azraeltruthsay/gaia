...
class LoopState:
    ...
    This captures information about detected loops for context preservation
    ...
    loop_type: Optional[str] = None            # LoopCategory value
    pattern: Optional[str] = None              # Human-readable pattern description
    pattern_hash: Optional[str] = None         # For deduplication

    # Escalation tracking
    reset_count: int = 0                       # Number of resets in this session
    confidence: float = 0.0                    # Detection confidence

    # Recovery context
    previous_attempts: List[LoopAttempt] = field(default_factory=list)
    recovery_context: Optional[str] = None     # Injected into prompt
    triggered_by: List[str] = field(default_factory=list)  # Which detectors fired

    # State flags
    in_recovery: bool = False                  # Currently in recovery mode
    warned: bool = False                       # Warning issued this session
    override_active: bool = False              # User override active
