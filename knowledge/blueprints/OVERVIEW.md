...
| **`gaia-prime`** | The Voice (Thinker Inference) | 7777 | 1x NVIDIA | gaia-engine-base |
...
- **Fallback chain**: thinker (gaia-prime) → groq_fallback (Groq API) → core (gaia-core embedded, CPU GGUF)

### Inference Backend Options

| Backend | Class | Location | Use Case |
|---------|-------|----------|----------|
| `thinker` / `gpu_prime` | VLLMRemoteModel | gaia-prime :7777 (GAIA Engine) | Primary GPU inference |
| `core` | VLLMRemoteModel | gaia-core :8092 (GAIA Engine, Qwen3.5-2B) | Medium tasks, safetensors GPU / GGUF CPU fallback |
| `reflex` | VLLMRemoteModel | gaia-nano :8080 (GAIA Engine) | Sub-second triage, classification |
...
|------------|-------------|
| **Hidden State Polygraph** | Layer-level activation access during forward passes — enables SAE feature mapping and ROME weight edits |
| **KV Cache Thought Snapshots** | `ThoughtHold` — snapshot and resume named thought contexts; `SegmentedKVManager` — incremental system prompt caching |
| **Static KV Cache** | Pre-allocated buffers eliminate per-token allocation overhead |
| **LoRA Adapter Management** | Load/unload/set_active via HTTP API (`/adapter/load`, `/adapter/set`) |
| **GPU↔CPU Migration** | Live device migration for watch rotation handoffs |
| **Vision Support** | Multimodal inference for Qwen3.5 VL models (auto-detected) |
