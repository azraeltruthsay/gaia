...
- **`models_list`** ✅ `http_rest` `/v1/models`  
  List models registered with vLLM server.
- **`chat_completions`** ✅ `http_rest` `/v1/chat/completions`  
  Chat completion — OpenAI-compatible API. Supports streaming, guided decoding (JSON/regex), and LoRA adapter selection.
- **`text_completions`** ✅ `http_rest` `/v1/completions`  
  Text completion — OpenAI-compatible API for raw text generation.
- **`vllm_sleep`** ✅ `http_rest` `/sleep`  
  vLLM sleep mode — offload KV cache to CPU RAM to reduce VRAM. Requires VLLM_SERVER_DEV_MODE=1.
- **`vllm_wake_up`** ✅ `http_rest` `/wake_up`  
  vLLM wake — restore KV cache from CPU RAM back to GPU VRAM.

## Volume Dependencies

- **gaia_warm_pool** `ro` → `/models` — Model weights and LoRA adapters (warm pool seeded at boot by systemd)

## Failure Modes

- 🟡 **gaia-prime unreachable**
  → gaia-core falls back to Groq API, then local GGUF model
- 🔴 **GPU out of memory (VRAM exhausted)**
  → vLLM rejects request with 503; requires container restart
- 🔴 **Model loading timeout (>120s)**
  → Health check fails; orchestrator does not bring service online
- 🟡 **vLLM process crash (OOM, CUDA error)**
  → Container exits; restart policy=unless-stopped restarts automatically
- 🟡 **LoRA adapter not found or mismatched**
  → Request fails; gaia-core catches exception and retries with base model
- 🔴 **CUDA architecture mismatch (wrong sm_ code)**
  → Container fails during vLLM init; orchestrator detects health check failure
