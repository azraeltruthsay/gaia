...
| `gaia-nano` | The Reflex — Nano triage classifier (GAIA Engine managed mode, GPU primary + GGUF fallback) | 8090 |
...
| `gaia-audio` | The Ears & Mouth — Qwen3-ASR STT, Nano-Refiner, TTS | 8080 |
...
|------|-------|------|---------|------|
| **Nano/Reflex** | Qwen3.5-0.8B-Abliterated | Qwen3.5-0.8B | gaia-nano GAIA Engine managed mode (GPU primary, GGUF CPU fallback) | Sub-second triage, transcript cleanup |
| **Core/Operator** | Qwen3.5-2B-GAIA-Core-v3 (identity-baked) | Qwen3.5-2B | GAIA Engine managed in gaia-core (GPU, port 8092) | Intent detection, tool selection, medium tasks |
| **Thinker/Prime** | Huihui-Qwen3-8B-GAIA-Prime-adaptive (identity-baked) | Qwen3-8B | GAIA Engine on gaia-prime (CPU/GGUF default, GPU when FOCUSING) | Complex reasoning, code, heavyweight tasks |
...
All three tiers (Nano, Core, Prime) run the **GAIA Engine** — a standalone library in its own GitHub repo at `github.com/azraeltruthsay/gaia-engine` (Apache-2.0). Key capabilities: hidden state polygraph, KV cache thought snapshots, LoRA adapter management, GPU↔CPU migration, vision support, ROME/SAE companion modules, GPU lifecycle state machine.
...
See `contracts/CONNECTIVITY.md` for the full matrix (90+ calls). See `contracts/services/*.yaml` for per-service API contracts.

Summary:
- **gaia-web → gaia-core**: HTTP POST `/process_packet` (primary), fallback to candidate
- **gaia-core → gaia-prime**: GAIA Engine API at `:7777` (Thinker CPU/GGUF default, GPU when FOCUSING, OpenAI-compatible, LoRA adapter support)
- **gaia-core → gaia-nano**: OpenAI-compatible API at `gaia-nano:8080` (Nano/Reflex triage)
- **gaia-core embedded**: GAIA Engine managed mode at `localhost:8092` (Core/Operator GPU inference)
