...
class CodexWriter:
    def __init__(self, config: Config, semantic_codex: SemanticCodex):
        ...
        self.self_generated_docs_root = Path(config.SHARED_DIR) / "self_generated_docs"
        try:
            self.self_generated_docs_root.mkdir(parents=True, exist_ok=True)
        except OSError:
            logger.warning("Cannot create self_generated_docs directory at %s; writing will be disabled", self.self_generated_docs_root)
