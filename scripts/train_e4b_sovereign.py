#!/usr/bin/env python3
"""
Train Gemma 4 E4B identity adapter — Sovereign Duality curriculum.

Trains a QLoRA adapter on the base Gemma 4 E4B model using the
sovereign-duality-e4b curriculum. Produces an adapter that can be
merged into the base model for deployment.

Usage:
    python scripts/train_e4b_sovereign.py

Output:
    /gaia/gaia-instance/gaia-models/lora_adapters/gemma4_e4b_sovereign_v2/
"""
...
# or on host (/gaia/gaia-instance/gaia-models/)
...
OUTPUT_DIR = f"{_BASE}/lora_adapters/gemma4_e4b_sovereign_v2"
MERGED_DIR = f"{_BASE}/Gemma4-E4B-GAIA-Core-v2"
...
def main():
    ...
    # Load text-only (skip vision encoder to save VRAM)
    model = AutoModelForCausalLM.from_pretrained(
        BASE_MODEL,
        quantization_config=bnb_config,
        device_map="auto",
        torch_dtype=torch.bfloat16,
        attn_implementation="eager",  # Flash attention can conflict with LoRA
    )
