...
class ImmuneSystem:
    """
    Analyzes system logs with semantic consolidation and triage.
    """
    ...
    def get_health_summary(self) -> str:
        try:
            stats = self._scan_and_triage()
            if not stats:
                return "Immune System: UNKNOWN (logs inaccessible)"
            
            total_unique_issues = sum(len(s["issues"]) for s in stats.values())
            total_raw_events = sum(s["raw_count"] for s in stats.values())
            systemic_score = sum(s["weighted_score"] for s in stats.values())
            
            if total_raw_events == 0:
                return "Immune System: STABLE. No active irritants."
            
            # Determine "Irritation" level based on systemic score
            if systemic_score > 20:
                state = "CRITICAL"
            elif systemic_score > 5:
                state = "IRRITATED"
            else:
                state = "MINOR NOISE"
            ...
            for service, data in stats.items():
                if data["raw_count"] > 0:
                    ...
                    short_issue = top_issue[:50] + "..." if len(top_issue) > 50 else top_issue
                    summary_parts.append(f"{service}: {len(data['issues'])} issues (Top: {short_issue})")
