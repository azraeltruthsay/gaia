...
Usage:
    from gaia_common.utils import render_gaia_packet_template
    from gaia_common.protocols import CognitionPacket

    text = render_gaia_packet_template(packet)
"""
...
from gaia_common.protocols import CognitionPacket, DataField
...
def _trim(value: Any, limit: int = MAX_INLINE_LEN) -> Any:
    """Trim string values to a maximum length."""
...
def _clean_dict(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Remove None, empty strings, empty lists, and empty dicts from payload."""
    return {k: v for k, v in payload.items() if v not in (None, "", [], {})}


def packet_to_template_dict(
    packet: CognitionPacket,
    processed_data_field_keys: Optional[Set[str]] = None
) -> Dict[str, Any]:
    """
    Return a nested dict representing the packet in a transport-friendly format.

    Args:
        packet: The CognitionPacket to convert
        processed_data_field_keys: Set of data field keys to skip

    Returns:
        Nested dictionary suitable for JSON serialization or template rendering
    """
    ...
    if context:
        ...
        context_payload = _clean_dict({
            "session_history_ref": getattr(
                getattr(context, "session_history_ref", None), "value", None
            ),
    ...
    content_payload = _clean_dict({
        ...
        "data_fields": _serialize_data_fields(
            getattr(content, "data_fields", []) if content else [],
            processed_data_field_keys
        ),
    })
    ...
    intent_payload = _clean_dict({
        ...
        "read_only": next(
            (bool(getattr(df, "value", False))
             for df in getattr(content, "data_fields", [])
             if getattr(df, "key", "") == "read_only_intent"),
            False
...
def render_gaia_packet_template(
    ...
    Args:
        packet: The CognitionPacket to render
        indent: Indentation string (default: two spaces)
        processed_data_field_keys: Set of data field keys to skip

    Returns:
        Human-readable text representation of the packet
    """
    template = packet_to_template_dict(packet, processed_data_field_keys)
    lines: List[str] = []

    def render_section(name: str, payload: Dict[str, Any]) -> None:
        if not payload:
            return
        lines.append(f"[{name}]")
    ...
    sections = (
        "header", "routing", "model", "context", "intent",
        "content", "reasoning", "governance", "metrics", "status"
    )
