...
FILE_LIST=$(
    find "${EXISTING_DIRS[@]}" \
        ...
        -type f \( \
            ...
            -name '*.yml' \
        \) -print | \
    grep -vE '__init__\.py$|_backup\.py$|\.py\.new$|\(backup\)|\.egg-info/|\.pyc$|\.lock$|\.map$|index_store\.json$|embeddings\.json$|final_prompt_for_review\.json$|last_activity\.timestamp$|sessions\.json$|/e2e_.*\.py$|/test_.*\.py$|/conftest\.py$|/setup\.py$|/README\.md$|/Dockerfile$|/dev_matrix\.json$|/sketchpad\.json$|/response_fragments\.json$|\.claude/' | \
